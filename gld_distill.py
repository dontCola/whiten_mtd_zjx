...
import utils
import metric
from load_cifar10 import get_dataloader
...
def main():
    ...
    # 加载CIFAR-10数据集 
    train_loader = get_dataloader('F:/whiten_mtd-main/datasets/cifar-10', batch_size=64, train=True) 
    val_loader = get_dataloader('F:/whiten_mtd-main/datasets/cifar-10', batch_size=64, train=False)

    ngpus_per_node = torch.cuda.device_count()

    mp.spawn(main_worker, nprocs=ngpus_per_node, args=(ngpus_per_node, args, train_loader, val_loader))


def main_worker(gpu, ngpus_per_node, args,train_loader, val_loader):
